...
func SetupRoutes(cfg *config.Config, registry *agents.Registry, sessionStore store.SessionStore, authSessionStore store.AuthSessionStore, cookieCrypto *auth.CookieCrypto, logger *zap.Logger, adminDeps *AdminDeps) http.Handler {
	...
	// Declared out here so the admin router can be wired into it below, once built.
	var mcpServerHandler *mcpserver.Handler
	...
	r.Route("/api", func(r chi.Router) {
		...
		// Admin API
		if adminDeps.UserService != nil {
			adminRouter := buildAdminRouter(cfg, adminDeps, registry, mcpRegistry, oidcClient, githubClient, authSessionStore, logger)
			r.Mount("/admin", adminRouter)

			// The MCP administration tools replay requests against this very router,
			// so they inherit its validation, role gating and audit trail.
			if mcpServerHandler != nil {
				mcpServerHandler.SetAdminRouter(adminRouter)
			}
